...
func AddTransactionAmountConsumer() {
	...
	for {
        ...
        newTransaction := models.Transaction{
				...
				transactionAmountAdmin := utils.ProtoConv(newTransaction)
				...
				if grpcResponse.IsAdded == "Success" {
					...
					_, err := transactionRepository.AddTransactionAmtToCustomerInDB(newTransaction)
					if err == nil {
						zap.L().Info("Successfully added transaction to customer from Orders Service"+customerId)
					} else {
						zap.L().Error("Error adding transaction points to Transaction DB"+err.Message)
					}
				} else {
					zap.L().Error("Error Updating Transaction Amount from Orders Service for the Customer through Admin GRPC")
				}
    	} else {
